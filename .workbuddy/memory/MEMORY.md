...
- **`title_body_qc.py`**（2026-09-08 加，第 4 套）：**标题 ⇄ 正文 主题一致性**守门。n-gram 覆盖率 < 0.08 判疑似错位（事故卡约 0.00-0.05，正确卡最低约 0.12）。**只抓硬错位，不追求标题被正文完整覆盖**——提炼式标题（"注资全景：3570亿怎么分"）用词本就不必逐字出现。跳过：有效长度 <6 的短分区标题 + `SKIP_WORDS` 章节名（深度解读/配置建议/今日复盘/关键数字速查等）。自检：对 20260908 事故版抓出 28 张，修复版 0 张。
...
## 7.5 新闻卡标题铁律（2026-09-08 立）
- **标题必须与正文同源**。曾因 `fetch_news.apply()` 用 akshare 真实头条覆盖 `*_标签和标题`、而正文键保留 agent 分析，导致 Tab1 **16/16 张、Tab2 21 张标题与正文全错**（标题讲"一线城市租金"，正文讲"A股深强沪弱"）。
- 现 `fetch_news.py` 默认 **不覆盖标题**（`TITLE_OVERRIDE`，env `LAOSHENG_NEWS_TITLE=1` 可强制开启）；真实新闻落 `/tmp/laosheng_news_*.json` 供 agent 查证事实，**不注入报告**。
- 生成时：标题在 `g_data2.py` 里与正文成对书写（同一 tuple），改正文必须同步改标题。
- 质检注意：违规检测暂无自动化，靠人工/用户反馈；如要加脚本，用「标题实词是否出现在正文」的覆盖率判定（阈值建议 0.4，但短正文截断会误判，须取全文比对）。
