...
## 周末/休市场景（2026-08-30 沉淀）
- T-1 = 最近交易日收盘（周日/周一生成 → 上周五收盘），页眉与 Tab3/5 标注「周末休市」。
- **市场数据可与上一期同源，但资讯区严禁沿用**：新闻/AI/机构观点/社区话题必须取周末（T-1 收盘后至生成时刻）的新事件重新撰写；本周主线事件、下周日历（PMI、非农、褐皮书、解禁、新股）是最好的"新鲜度"素材。

## 生成器工程约定 · 分文件架构（2026-08-30 沉淀）
- 拆分为 `gen`（TAB0-1）、`gen2`（TAB2-4）、`gen3`（TAB4-7）三个数据层 + `render`（渲染+后处理）+ `boost`（高亮）+ `qc`（质检），比单文件好维护。
- **中文引号铁律**：数据层 Python 文件里，正文中的引号一律用中文 “ ”，只有 Python 字符串定界符用 ASCII `"`。用 ASCII `"` 包中文短语会直接 SyntaxError。
  - 应急修复：只对「前后均为 CJK/全角」的 `"` 交替替换为 “ ”；**禁止**对裸词做全局 replace（会误伤 `S("保险","up")` 这类参数）。
- **booster 两个必守 bug**：
  1. 判断可高亮必须写 `if all(stack)`，不能写 `if stack and all(stack)`——空栈时后者为 False，会导致**全文不高亮**。
  2. 只跟踪 `<span>` 不够，必须跟踪**所有容器标签**（配 VOID 集合排除 `img/br/path/line` 等自闭合标签），并用 `NO_HL_CLASSES`（hm-sub、hm-ticker、tab-btn、brand-*、stock-name、section-title、sub-title、topic-tag…）跳过页眉/导航/标题等小字区域。
- **QC 检查点修正**：
  - 情绪条宽度正则用 `style="width:([^;"]+)[;"]`（其后是 `;` 不是 `"`）。
  - 统计正文 `--` 前须先剔除 `<!-- -->` 注释与 `<style>` 块（否则 123 条注释 = 246 处误报）。
  - 「老盛观点」计数用 `class="lv-label">老盛观点：`（CSS 注释里也有该词，直接 count 会得到 12 而非 5）。
  - 问句检查须先 strip 标签（booster 会在标题内插 span）。
  - `速查N_标签class` 必须与数值符号一致（本期上证 -0.11% 误写 up，被「class="up" 无负值」抓出）。
