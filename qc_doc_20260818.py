# -*- coding: utf-8 -*-
"""严格对照 报告质量检查.md(v2.3) 的 QC 脚本（修正版）
...
check("一.及时性", "新数据(8/17)≥10处", html.count("8/17") >= 10, f"8/17出现{html.count('8/17')}处")
# 数据一致性：关键指数点位全文件一致
sh = html.count("3982.65"); sz = html.count("14704.27")
check("一.及时性", "上证3982.65点位一致", sh >= 2, f"出现{sh}次")
check("一.及时性", "深证14704.27点位一致", sz >= 2, f"出现{sz}次")
...
check("三.标注", "存在8/17收盘标注", "8/17收盘" in html, f"'8/17收盘'={'8/17收盘' in html}")
...
miss_m = [k for k in req_markets if f'class="market-name">{k}<' not in html]
check("三.完整性", "Tab3各市场market-name已填充", len(miss_m) == 0, f"缺{len(miss_m)}: {miss_m}")
# 涨跌幅 span 存在且无 --
miss_chg = [k for k in req_markets
            if re.search(r'class="market-name">'+re.escape(k)+r'<.*?</span>\s*</span>\s*</div>',
                         html, re.S) is None]
# 简化：检查每个 market-name 同行或邻近有 font-weight:700 的涨跌幅
miss_chg2 = []
for k in req_markets:
    seg = re.search(r'class="market-name">'+re.escape(k)+r'<.*?</div>\s*</div>', html, re.S)
    if not seg or 'font-weight:700' not in seg.group(0):
        miss_chg2.append(k)
check("三.完整性", "各市场涨跌幅已填充(非--)", len(miss_chg2) == 0, f"缺{len(miss_chg2)}: {miss_chg2[:6]}")

# 个股股价
req_stocks = ["7.56","10.30","6.46","38.20","17.83","11.10","32.19","11.62","16.25",
              "10.85","27.96","4.70","96.02","8.77","51.80"]
miss_s = [v for v in req_stocks if v not in html]
check("三.完整性", "13只关注标的股价已填充", len(miss_s) == 0, f"缺{len(miss_s)}: {miss_s}")

# 估值参考（模板实际字段）
est = ["银行板块PB","上证PE","银行股息率","中证红利PE","神华吨煤利润",
       "中国移动PB","招商银行PB","中国核电PE","黄金价格"]
est_missing = [k for k in est if k not in html]
