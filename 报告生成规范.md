...
1. **4轮搜索**（每轮最多6个搜索，总计不超过24次）：
   ...
   - **第2轮**：港股/亚太/欧洲最近一次收盘 + 大宗商品（原油/黄金/白银）+ 加密货币 + 汇率/国债
   - **第3轮A**：最新AI前沿（大模型/机器人/算力/应用）+ 关注标的基本面 + 最新论坛热门话题
   - **第3轮B**：最新券商策略观点 + 板块估值 + 存款利率 + 理财收益率 + 保险产品动态
...
4. **更新index.html**：在reports数组最前面添加新日期条目（格式参考已有条目），index.html密码为 `12345678`。**历史记录只保留最近10天（不包含当天）**，超过10天的旧记录需删除。
